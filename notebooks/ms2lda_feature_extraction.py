...
class Loader(object):
    ...
    def process_peaklist(self, ms1, ms2, metadata):
        ...
        print(ms1[0], list(metadata.values())[0])
        ...
        for n_peaks_checked,peak in enumerate(self.ms1_peaks):
            ...
            if featid != None and featid in featid_ms1_dict:
                ...
                ms1_hits = [x for x in ms1 if x.mz >= min_mz and x.mz <= max_mz and x.rt >= min_rt and x.rt <= max_rt]
            ...
            if ',' in peak[3]:
                ...
                new_metadata[new_ms1.name]['intensities'] = dict(list(zip(self.sample_names, tokens)))
    ...
    def filter_ms1_intensity(self,ms1,ms2,min_ms1_intensity = 1e6):
        ...
        ms2 = [x for x in ms2 if x[3] in set(ms1)]
    ...
    def filter_ms2_intensity(self,ms2, min_ms2_intensity = 1e6):
        ...
        ms2 = [x for x in ms2 if x[2] >= min_ms2_intensity]
        print("{} MS2 remaining".format(len(ms2)))
        return ms2
    ...
    def filter_ms1(self,ms1,ms2,mz_tol = 0.5,rt_tol = 16):
        ...
        while True:
            ...
            hits = [x for x in ms1_by_intensity if x.mz > min_mz and x.mz < max_mz and x.rt > min_rt and x.rt < max_rt]
            for hit in hits:
    ...
    def load_spectra(self,input_set):
        ...
        with open(input_set,'r') as f:
            ...
            for line in f:
                ...
                # Add the intensities
                self.metadata[doc_name]['intensities'] = dict(list(zip(sample_names,[float(i) for i in tokens[17:23]])))
            ...
            for spectrum in run:
                if spectrum['ms level'] == 1:
                    ...
                    # Note can sometimes get empty scans at the start. If this happens we should ignore.
                        ...
                        current_ms1_scan_mz,current_ms1_scan_intensity = list(zip(*spectrum.peaks))
                    else:
                        current_ms1_scan_mz = None
                        current_ms1_scan_intensity = None
                        ...
                        if rline.startswith('>'):
                            ...
                            if not rline.startswith('>collision'):
                                ...
                                if in_doc:
                                    ...
                                    doc_name = doc_name_prefix + '_collision_' + str(current_collision)
...
class LoadMSP(Loader):
    ...
    def load_spectra(self,input_set):
        ...
        for input_file in input_set:
            ...
            for key, value in list(ms2_dict.items()):
            ...
            for inchikey, value in list(inchikey_ms2_dict.items()):
                ...
                for mz, intensity in list(temp_mz_intensity_dict.items()):
...
class MS2LDAFeatureExtractor(object):
    ...
    def get_first_corpus(self):
        first_file_name = list(self.corpus.keys())[0]
